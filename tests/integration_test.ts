...
// Integration test for migrate command (dry-run)
Deno.test({
  name: "Integration: dpp migrate - dry run",
  async fn() {
    const tempDir = await Deno.makeTempDir();

    try {
      // Create a sample dein config
      const deinConfig = `
call dein#add('Shougo/ddu.vim')
call dein#add('Shougo/ddc.vim', {'on_cmd': ['Ddc']})
`;
      const configPath = join(tempDir, "init.vim");
      await Deno.writeTextFile(configPath, deinConfig);

      // Run migrate command with dry-run
      const migrateCommand = new Deno.Command(Deno.execPath(), {
        args: [
          "run",
          "--allow-read",
          "--allow-write",
      ...
      const { code, stdout, stderr } = await migrateCommand.output();
      const output = new TextDecoder().decode(stdout);
      const errorOutput = new TextDecoder().decode(stderr);
      ...
      assertEquals(code, 0);
      assertEquals(output.includes("Migrating from dein"), true);
      assertEquals(output.includes("DRY RUN"), true);
      assertEquals(output.includes("Shougo/ddu.vim"), true);
    } finally {
      try {
        await Deno.remove(tempDir, { recursive: true });
      } catch {
